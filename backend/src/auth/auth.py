...
AUTH0_DOMAIN = 'dev-4yewlodtqwpxz0xf.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    ...
    splits = authentication.split()
        
    if splits[0].lower() != "bearer":
        raise AuthError({
            'code': 'bearer_missing',
            'description': 'Bearer is expected.'
        }, 401)
...
def check_permissions(permission, payload):
    ...
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'not permit',
            'description': 'You dont have permission to do this.'
        }, 401)
